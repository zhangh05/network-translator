...
FIREWALL_CAP = {
    FeatureKey.ZONE: FeatureSupport(FeatureSupportStatus.FULL, notes="Topsec zone model"),
    ...
    FeatureKey.SECURITY_POLICY: FeatureSupport(FeatureSupportStatus.FULL, modes=["interzone"]),
    ...
    FeatureKey.OSPF: FeatureSupport(FeatureSupportStatus.PARTIAL, notes="OSPF support varies by model"),
    FeatureKey.BGP: FeatureSupport(FeatureSupportStatus.PARTIAL),
...
profile = VendorPlatformProfile(
    ...
    interface_naming=InterfaceNaming(
        pattern=r"(?i)(GigabitEthernet|FastEthernet|Vlan|Loopback|Tunnel|NULL)\d",
        svi_prefix="Vlan",
        ...
        physical_patterns=["GigabitEthernet", "FastEthernet"],
    ),
    signatures=[
    ...
    forbidden_patterns=[
        ForbiddenPattern(pattern=r"(?i)switchport", severity=IRRiskLevel.HIGH, category=ForbiddenPatternCategory.RESIDUAL_SYNTAX, message="Switch command in Topsec firewall config", target_context="interface"),
    ...
    limitations=[
        VendorLimitation(title="Limited routing", description="OSPF/BGP support varies by Topsec model", domain=DeviceDomain.FIREWALL),
